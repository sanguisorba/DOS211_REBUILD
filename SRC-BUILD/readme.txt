...
John Elliott氏のアセンブリセットから生成した ”STDSWをIBMではなくマイクロソフトにした” MSDOS.EXEをEXE2BINでIBMDOS.COMに変換し、試しにEPSON MS-DOS 2.11のディスクに非IBMスイッチのCOMMAND.COMと一緒に置換したところ、普通に起動できました。

本来は著作権表示が`Copyright (c) 1981-1983, 1985 by Microsoft and EPSON`で、IBMDOS.COMが著作権表示を行っているのですが、ビルドしたIBMDOS.COMではマイクロソフトのみになっています。COMMAND.COMによるメッセージ出力はその下の`Command v. 2.11`からになります。
